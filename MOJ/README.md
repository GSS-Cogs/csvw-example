### MOJ: 5 Star Linked Open Data

This document is a walkthrough of how we can represent MOJ data with csvw, in such a way as it supports conversion to RDF.
...
For our observation file we're using the included file `probation-caseload_all-supervision.csv`. A copy of this file is present in all examples.
...
- I've changed `start_date` and `end_date` to a sngle entry of `Period` and reformmatted as per: https://github.com/epimorphics/IntervalServer/blob/master/interval-uris.md#uri-set-and-dataset-descriptions. For example `day/2018-03-31`

- I've replaced `supervision_type`, `value_description` and `value_type` with a `Measure Type` column containing a measure of `Count of offenders supervised`.

- I've rename `CTRY18CD` to `Area`.

- I've removed `nps_region` and `nps_crc`.
